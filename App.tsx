...
const App: React.FC = () => {
  ...
  const handleRegister = async (payload: { name: string; email: string; password: string; role: UserRole }) => {
    try {
      ...
      setUser({
        id: isAdmin ? 'a1' : isTeacher ? 't1' : 's1',
        name: payload.name || (isAdmin ? 'Admin User' : isTeacher ? 'Sarah Jenkins' : 'Alex Student'),
        email:
          payload.email ||
          (isAdmin ? 'admin@edustream.com' : isTeacher ? 'sarah@edustream.com' : 'student@edustream.com'),
        role: payload.role,
        avatar: `https://i.pravatar.cc/150?u=${payload.email || (isAdmin ? 'admin' : isTeacher ? 'teacher' : 'student')}`,
        subscription: isTeacher || isAdmin ? undefined : SubscriptionPlan.FREE,
      });
    }
