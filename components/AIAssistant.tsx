...
const AIAssistant: React.FC = () => {
  ...
  // Detect if we are in the classroom based on a simplified check (or we could pass a prop)
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: '1',
      sender: 'ai',
      content: 'I am tracking this live session. Ask me anything about the current topic!',
      timestamp: new Date()
    }
  ]);
  ...
  const handleSend = async () => {
    ...
    const responseText = await getAiTutorResponse(input, history);
    ...
    <div className={`bg-slate-900 rounded-[2.5rem] shadow-2xl border border-white/5 flex flex-col overflow-hidden h-full ${!isOpen && !isClassroom ? 'hidden' : ''}`}>
      <div className="p-4 bg-indigo-600 text-white flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-xs tracking-tight">AI Study Buddy</h3>
            <p className="text-[8px] text-indigo-100 font-bold uppercase tracking-widest">Context: Lesson 04</p>
          </div>
        </div>
      </div>
